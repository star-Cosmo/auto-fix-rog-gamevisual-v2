...
| 厂商代码 | 厂商 | EDID[8..11] | 计算出的 ID | 实际使用 ID |
|---|---|---|---|---|
| CSW | 国产面板商（替换屏市场常见） | `0E 77 0F 15` | `770E150F` | `150F` |
| BOE | 京东方 | `09 E5 07 0A` | `E5090A07` | `0A07` |
| E430 | LG Display（乐金显示，LGD） | `30 E4 63 05` | `E4300563` | `0563` |
| AF06 | 友达光电（AUO） | `06 AF A2 D2` | `AF06D2A2` | `D2A2` |
| AE0D | 群创光电（Innolux） | `0D AE 3C 15` | `AE0D153C` | `153C` |
| 104D | 夏普（SHP） | `4D 10 59 15` | `104D1559` | `1559` |
| B851 | 天马（TMX） | `51 B8 61 15` | `B8511561` | `1561` |
| 6F0E | 华星光电（CSO / CSOT） | `0E 6F 0F 15` | `6F0E150F` | `150F` |

> 说明：
> - **实际使用 ID** 即产品号（后 4 位）——工具优先匹配完整 8 位硬件 ID，匹配不到时按产品号兜底，所以它才是决定"能不能找到文件"的关键。
> - CSW / BOE / E430 三行来自实机 EDID 实测；AF06 / AE0D / 104D / B851 / 6F0E 来自社区 ICC 库中前人验证过的文件反推。
> - 同一厂商在 EDID 里是 3 个字母（如 SHP），在文件名里是 4 位十六进制前缀（如 104D），两者是同一事物的两种编码表现。
> - 有趣的冷知识：`6F0E150F` 是华星光电 150F 面板的官方文件——很多 FX507ZM 出厂就带这种屏；如果你的替换屏是 CSW 的，这份文件虽然产品号相同，厂商前缀不对，奥创照样不认。
